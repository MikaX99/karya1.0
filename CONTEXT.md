...
**Last Updated:** 2026-09-17 (dini hari: logo hi-res, audit keamanan, default branch)

---

## Active Checkpoint
- **Status:** Compro **LIVE** di GH Pages = `39af09e` (deploy 2026-09-17, logo klien 320 px + Next 16.3.5, audit 0). `main` = `origin/main`. Default branch GitHub kini **`main`**; `demo19` (7 commit awal 2026-08-16, riwayat terpisah) diarsip lokal sebagai `demo19-arsip`, dihapus di GitHub oleh Mika. Tidak ada Docker/script deploy lokal lagi — deploy hanya via Actions. Remote kini **SSH** (`git@github.com:MikaX99/karya1.0.git`, kunci `id_ed25519` terdaftar 2026-09-17; `gh` tetap belum login). Compro **selesai 2026-08-19** (HEAD `3ccf454`, 138 commit, semua compro). Deploy GitHub Pages via Actions (`MikaX99/karya1.0`, branch `gh-pages`, basePath `/karya1.0`). Tumpahan KBLI/RAG (insiden sesi salah workspace, Agustus 2026) dipisah ke branch `kbli-spill` 2026-09-17 (disimpan, lokal).
...
- **Riwayat singkat:**
  - 2026-09-17 [SELESAI] Handoff `logo-klien-hires`: 13 logo dari sentracomp.com → 320 px, PNG palet (`public/clients/` 140 KB); client-07 → "PLN Energi Gas" (keputusan Mika). `c01ae26`.
  - 2026-09-17 [SELESAI] Handoff `audit-keamanan`: Next/eslint-config-next 16.3.5, sharp 0.35.4, js-yaml 4.3.2 → `npm audit` 0; Dockerfile/docker-compose/.dockerignore & `scripts/deploy.js` dihapus (keputusan Mika). `05ab3c4`, `86a1474`, `39af09e`. Sisa untuk Mika: `Archive.zip` 126 MB untracked di root → pindah/hapus sendiri. Catatan: `npm i` memblokir postinstall `unrs-resolver` (allowScripts) — lint tetap jalan (173 problem warisan).
  - 2026-09-17 [SELESAI] Deploy `17c5c08..39af09e` via SSH → Actions → `gh-pages` `02cb2eb`; live: PLN Energi Gas tampil, 0 nama lama, logo baru tersaji. Default branch repo `demo19` → `main` (Mika via web; pane PR app sebelumnya salah tampil +17.611 karena membandingkan ke `demo19`).
